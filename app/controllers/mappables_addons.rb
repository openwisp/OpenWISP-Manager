module MappablesAddons
  ...
  def get_wisp_geocode(wispAddress)
    env = ENV['RAILS_ENV'] || RAILS_ENV
    geocode = Geocoding::get(wispAddress, :key => YAML.load_file(RAILS_ROOT + '/config/gmaps_api_key.yml')[env])
    if geocode.status == Geocoding::GEO_SUCCESS
      return [geocode[0].latitude, geocode[0].longitude]
    else
      return [0,0]
    end
